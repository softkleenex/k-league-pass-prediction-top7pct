"""
K리그 패스 좌표 예측 - 8x8 Zone 모델

Week 1 실험: Zone 해상도 최적화
- 8x8 grid (64 zones)
- 8-way direction
- min_samples Grid Search [22, 25, 28]
...
print("=" * 80)
print("K리그 패스 좌표 예측 - 8x8 Zone 모델")
print("=" * 80)
...
def get_zone_8x8(x, y):
    """8x8 Zone 분류 (64 zones)"""
    x_zone = min(7, int(x / (105 / 8)))
    y_zone = min(7, int(y / (68 / 8)))
    return x_zone * 8 + y_zone
...
def build_model_8x8(df, min_samples):
    """8x8 Zone + 8-way Direction 모델"""
    df = df.copy()
    df['zone'] = df.apply(lambda r: get_zone_8x8(r['start_x'], r['start_y']), axis=1)
...
def predict_8x8(row, model):
    """예측"""
    zone = get_zone_8x8(row['start_x'], row['start_y'])
...
min_samples_candidates = [22, 25, 28]
results = []
...
for min_s in min_samples_candidates:
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
        ...
        # 모델 구축
        model = build_model_8x8(train_fold, min_s)

        # 예측
        predictions = val_fold.apply(lambda r: predict_8x8(r, model), axis=1)
...
final_model = build_model_8x8(train_last, best_min_samples)

predictions = test_last.apply(lambda r: predict_8x8(r, final_model), axis=1)
...
submission.to_csv('/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_8x8_zone.csv', index=False)

print("  submission_8x8_zone.csv 저장 완료")
...
print(f"  8x8 모델:      {public_estimate:.4f} (예상)")
