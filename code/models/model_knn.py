...
print(f"[{datetime.now().strftime('%H:%M:%S')}] KNN 시작")
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(train, groups=train['game_id'])):
    ...
    model_x = KNeighborsRegressor(n_neighbors=50, weights='distance', n_jobs=-1)
    model_x.fit(X_train, y_train_x)
    
    model_y = KNeighborsRegressor(n_neighbors=50, weights='distance', n_jobs=-1)
...
with open('checkpoints/knn/results.json', 'w') as f:
    json.dump({'cv_fold_1_3': float(cv), 'fold_scores': [float(s) for s in fold_scores[:3]]}, f)
print(f"[{datetime.now().strftime('%H:%M:%S')}] KNN 완료!")
