"""
K리그 패스 좌표 예측 - 최적화 앙상블 모델

전략:
1. Pending 파일 중 가장 유망한 모델들을 앙상블
2. 8방향 + 5방향 혼합 전략
3. Zone 해상도 다양화 (5x5, 6x6, 7x7, 8x8)
4. CV 16.2+ 유지하면서 성능 최대화

예상 결과:
- CV: 16.05-16.15 (안전 구간 상단)
- Gap: +0.20-0.25
- Public: 16.25-16.40
"""
...
for top_n in [3, 4, 5]:
    ...
    variances = [np.var(cv_scores[idx]) + 1e-8 for idx in top_models]
...
if final_cv >= 16.2:
    gap_estimate = 0.17
    risk_level = "낮음 (안전)"
    public_estimate = final_cv + gap_estimate
    recommend = "즉시 제출 권장"
    color = "GREEN"
elif final_cv >= 16.0:
    gap_estimate = 0.25
...
submission_path = '/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_optimized_ensemble.csv'
...
print(f"\n[기준 모델과 비교]")
print(f"  현재 Best:     CV 16.03 → Public 16.3574 (Gap +0.33)")
print(f"  신규 모델:     CV {final_cv:.4f} → Public {public_estimate:.2f} (예상 Gap +{gap_estimate:.2f})")
print(f"  CV 차이:       {16.03 - final_cv:+.4f}")
print(f"  Public 개선:   {16.3574 - public_estimate:+.4f} (예상)")
