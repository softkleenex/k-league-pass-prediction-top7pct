"""
K리그 패스 좌표 예측 - 7x7 Zone 미세 조정

Week 1 실험: 7x7 min_samples 최적화
- 7x7 grid (49 zones)
- 8-way direction
- min_samples Grid Search [18, 20, 22, 24, 26]
...
print("=" * 80)
print("K리그 패스 좌표 예측 - 7x7 Zone 미세 조정")
print("=" * 80)
...
def build_model_7x7(df, min_samples):
    """7x7 Zone + 8-way Direction 모델"""
    df = df.copy()
    df['zone'] = df.apply(lambda r: get_zone_7x7(r['start_x'], r['start_y']), axis=1)
...
def predict_7x7(row, model):
    """예측"""
    zone = get_zone_7x7(row['start_x'], row['start_y'])
...
# =============================================================================
# 5. min_samples Grid Search (미세 조정)
# =============================================================================
print("\n[5] min_samples Grid Search (미세 조정)...")
...
min_samples_candidates = [18, 20, 22, 24, 26]
results = []
...
for min_s in min_samples_candidates:
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
        ...
        # 모델 구축
        model = build_model_7x7(train_fold, min_s)

        # 예측
        predictions = val_fold.apply(lambda r: predict_7x7(r, model), axis=1)
...
final_model = build_model_7x7(train_last, best_min_samples)

predictions = test_last.apply(lambda r: predict_7x7(r, final_model), axis=1)
...
submission.to_csv('/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_7x7_tuned.csv', index=False)

print("  submission_7x7_tuned.csv 저장 완료")
...
print(f"  7x7 tuned:     {public_estimate:.4f} (예상)")
print(f"  개선:          {16.3639 - public_estimate:+.4f}")

print(f"\n[기존 7x7과 비교]")
print(f"  기존 7x7 (min_samples=20): CV 16.66")
print(f"  tuned 7x7: CV {best_result['fold13_cv']:.4f}")

if public_estimate < 16.3639 and best_result['fold13_std'] < 0.01:
    print(f"\n✅ 제출 권장!")
    ...
    if public_estimate >= 16.3639:
        print(f"  - 예상 Public이 현재 Best보다 높음")
    if best_result['fold13_std'] >= 0.01:
        print(f"  - Fold 분산 높음 (>= 0.01)")
