"""
K리그 패스 좌표 예측 - Fold 1-3 기반 안전 모델

전략:
1. Fold 4-5는 실제로 예측하기 쉬운 데이터 (거리 짧음, 분산 작음)
2. Public 데이터는 Fold 1-3과 유사할 가능성 높음
3. Fold 1-3 평균 CV를 진짜 성능으로 간주
4. 여러 Zone 해상도 + 방향 조합으로 안전하게 앙상블

목표:
- Fold 1-3 CV: 16.25-16.35
- 예상 Public: 16.42-16.52 (Gap +0.17)
"""
...
print("=" * 80)
print("K리그 패스 좌표 예측 - Fold 1-3 기반 안전 모델")
print("=" * 80)
    ...
    for model in models:
        ...
        def predict_row(row):
            ...
            if key in stats.index and stats.loc[key, 'count'] >= min_s:
                ...
                dx = zone_fallback['delta_x'][row['zone']]
...
submission.to_csv('submission_safe_fold13.csv', index=False)

print("  submission_safe_fold13.csv 저장 완료")
...
print(f"  예상 Public: {fold13_cv + 0.17:.2f} (Gap +0.17 가정)")

print(f"\n[전략]")
print(f"  ✅ Fold 4-5 제외 (쉬운 데이터, 과적합 위험)")
print(f"  ✅ Fold 1-3 기반 Inverse Variance Weighting")
print(f"  ✅ 여러 Zone 해상도 + 방향 조합")
print(f"  ✅ 계층적 Fallback (안정성)")

print(f"\n[제출 파일]")
print(f"  submission_safe_fold13.csv")
