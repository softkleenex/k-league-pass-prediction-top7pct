"""
K리그 패스 좌표 예측 - 9x9 Zone 모델

Week 1 실험: Zone 해상도 최적화
- 9x9 grid (81 zones)
- 8-way direction
- min_samples Grid Search [25, 28, 30]
- Fold 1-3 CV 목표: 16.29-16.33
...
print("=" * 80)
print("K리그 패스 좌표 예측 - 9x9 Zone 모델")
print("=" * 80)
...
def get_zone_9x9(x, y):
    """9x9 Zone 분류 (81 zones)"""
    x_zone = min(8, int(x / (105 / 9)))
    y_zone = min(8, int(y / (68 / 9)))
    return x_zone * 9 + y_zone
...
def build_model_9x9(df, min_samples):
    """9x9 Zone + 8-way Direction 모델"""
    df = df.copy()
    df['zone'] = df.apply(lambda r: get_zone_9x9(r['start_x'], r['start_y']), axis=1)
...
def predict_9x9(row, model):
    """예측"""
    zone = get_zone_9x9(row['start_x'], row['start_y'])
...
min_samples_candidates = [25, 28, 30]
results = []
...
for min_s in min_samples_candidates:
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
        ...
        # 모델 구축
        model = build_model_9x9(train_fold, min_s)

        # 예측
        predictions = val_fold.apply(lambda r: predict_9x9(r, model), axis=1)
...
final_model = build_model_9x9(train_last, best_min_samples)

predictions = test_last.apply(lambda r: predict_9x9(r, final_model), axis=1)
...
submission.to_csv('/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_9x9_zone.csv', index=False)

print("  submission_9x9_zone.csv 저장 완료")
...
print(f"  9x9 모델:      {public_estimate:.4f} (예상)")
