"""
K리그 패스 좌표 예측 - LSTM v2 Sampling (10% 빠른 검증)
On-the-fly 시퀀스 생성 + Episode 단위 샘플링

목표: 2-3시간 내 CV 점수 확인 (가능성 판단용)
"""
...
# ⭐ 샘플링 설정 (10% - 빠른 검증용)
SAMPLE_FRAC = 0.10  # 10% 샘플링
RANDOM_STATE = 42

print(f"Device: {DEVICE}")
print(f"Sequence Length: {SEQ_LEN}")
print(f"Batch Size: {BATCH_SIZE}")
print(f"⭐ Sampling: {SAMPLE_FRAC*100:.0f}% (빠른 검증용)")
print()
...
# ⭐ 10% 샘플링 (episode 단위로 샘플링)
print(f"  원본: {len(train_full):,} samples, {train_full['game_episode'].nunique():,} episodes")
episode_sample = train_full['game_episode'].drop_duplicates().sample(frac=SAMPLE_FRAC, random_state=RANDOM_STATE)
train = train_full[train_full['game_episode'].isin(episode_sample)].reset_index(drop=True)
print(f"  샘플링: {len(train):,} samples, {train['game_episode'].nunique():,} episodes ({SAMPLE_FRAC*100:.0f}%)")
print()
...
class PassSequenceDataset(Dataset):
    ...
    def __init__(self, df, seq_len=3):
        ...
        # test.csv에는 time_seconds가 없을 수 있음
        if 'time_seconds' in df.columns:
            df_sorted = df.sort_values(['game_episode', 'time_seconds']).reset_index(drop=True)
        else:
            df_sorted = df.sort_values(['game_episode']).reset_index(drop=True)
