"""
K리그 패스 좌표 예측 - 패스 거리 조건부 Zone 모델

핵심 아이디어:
1. 패스 거리 분류 (짧은/중간/긴 패스)
   - short: < 10m
   - medium: 10-25m
   - long: > 25m

2. 방향(8방향) + 거리(3단계) = 24개 조합 그룹
   - 각 그룹별 Zone 통계 적용

3. min_samples 동적 조정 (그룹 크기에 따라 15-30)

4. 앙상블: 5x5, 6x6, 7x7 Zone 조합
...
print("=" * 80)
print("K리그 패스 좌표 예측 - 패스 거리 조건부 Zone 모델")
print("=" * 80)
...
# 패스 거리 분포 분석
print(f"\n  패스 거리 분포:")
print(f"    Min: {train_last['prev_distance'].min():.2f}m")
print(f"    Max: {train_last['prev_distance'].max():.2f}m")
print(f"    Median: {train_last['prev_distance'].median():.2f}m")
print(f"    Mean: {train_last['prev_distance'].mean():.2f}m")

# =============================================================================
# 3. Zone, 방향, 거리 분류 함수
# =============================================================================
print("\n[3] Zone, 방향, 거리 분류 함수 정의...")
...
def get_adaptive_min_samples(zone, n_zones, group_count, base=25):
    """
    동적 min_samples 조정

    그룹이 세분화될수록 threshold를 낮춤
    공격 지역에서는 threshold를 더 낮춤
    """
    # 기본 threshold
    threshold = base
    ...
    # 그룹 데이터가 충분하면 threshold를 더 낮춤 (과적합 방지)
    # 하지만 너무 낮추면 안됨 (CV < 16.2 위험)
    if group_count >= 100:
        threshold = int(threshold * 0.85)  # 최대 15% 감소

    # 최소값 보장 (15 이하로 내려가지 않음)
...
def build_distance_model(df, n_x, n_y):
    """패스 거리 조건부 Zone 통계 구축"""
    df = df.copy()
    ...
    # 3가지 레벨의 통계 구축
    # Level 1: Zone only (가장 안정적)
    zone_stats = df.groupby('zone').agg({
        'delta_x': 'median',
        'delta_y': 'median'
    }).to_dict()

    # Level 2: Zone + Direction (중간 세분화)
    ...
    # Level 3: Zone + Direction + Distance (가장 세분화)
...
def predict_distance_zone(row, model):
    """패스 거리 조건부 예측 (계층적 Fallback)"""
    ...
    # Level 3 시도: Zone + Direction + Distance
    key_full = f"{zone}_{direction}_{distance_cat}"
    full_count = model['zone_dir_dist_count'].get(key_full, 0)

    # 동적 threshold
    min_samples_full = get_adaptive_min_samples(zone, model['n_x'] * model['n_y'], full_count, base=25)

    if key_full in model['zone_dir_dist_x'] and full_count >= min_samples_full:
        dx = model['zone_dir_dist_x'][key_full]
        dy = model['zone_dir_dist_y'][key_full]
    else:
        # Level 2 Fallback: Zone + Direction
        key_dir = f"{zone}_{direction}"
        dir_count = model['zone_dir_count'].get(key_dir, 0)
        min_samples_dir = get_adaptive_min_samples(zone, model['n_x'] * model['n_y'], dir_count, base=20)

        if key_dir in model['zone_dir_x'] and dir_count >= min_samples_dir:
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
    ...
    for n_x, n_y in ZONE_CONFIGS:
        model = build_distance_model(train_fold, n_x, n_y)

        predictions = val_fold.apply(
            lambda r: predict_distance_zone(r, model),
            axis=1
        )
...
for config in ZONE_CONFIGS:
    ...
    print(f"  {config[0]}x{config[1]} Distance:  {mean_score:.4f} ± {std_score:.4f}")

# 과적합 위험 평가
best_cv = min(np.mean(cv_scores[config]) for config in ZONE_CONFIGS)
print(f"\n과적합 위험 평가:")
if best_cv >= 16.2:
...
models = {}
for n_x, n_y in ZONE_CONFIGS:
    models[(n_x, n_y)] = build_distance_model(train_last, n_x, n_y)

# 전체 데이터에 대한 예측
for n_x, n_y in ZONE_CONFIGS:
    predictions = train_last.apply(
        lambda r: predict_distance_zone(r, models[(n_x, n_y)]),
        axis=1
    )
...
for w1 in np.arange(0.15, 0.35, 0.05):
    for w2 in np.arange(0.40, 0.60, 0.05):
        w3 = 1 - w1 - w2
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
    ...
    fold_models = {}
    for n_x, n_y in ZONE_CONFIGS:
        fold_models[(n_x, n_y)] = build_distance_model(train_fold, n_x, n_y)

    for n_x, n_y in ZONE_CONFIGS:
        predictions = val_fold.apply(
            lambda r: predict_distance_zone(r, fold_models[(n_x, n_y)]),
            axis=1
        )
        val_fold[f'pred_x_{n_x}x{n_y}'] = predictions.apply(lambda x: x[0])
        val_fold[f'pred_y_{n_x}x{n_y}'] = predictions.apply(lambda x: x[1])
...
# 제출 디렉토리 확인
submission_path = Path("submission_pass_distance.csv")
...
sub_6x6_path = Path("submission_pass_distance_6x6_only.csv")
sub_6x6.to_csv(sub_6x6_path, index=False)
print(f"  {sub_6x6_path} 저장 완료 (CV: {np.mean(cv_scores[(6,6)]):.4f})")
...
print(f"  6x6 단독 CV:   {np.mean(cv_scores[(6,6)]):.4f}")
print(f"  과적합 위험:   {risk}")

print(f"\n[기준 모델과 비교]")
print(f"  8방향 모델:    CV 16.03 → Public 16.36 (Gap +0.33)")
print(f"  Distance 모델: CV {final_cv:.4f} → Public ??? (예상 Gap +0.20-0.30)")
print(f"  CV 차이:       {final_cv - 16.03:+.4f}")
...
if final_cv >= 16.2:
    print(f"  ✅ 즉시 제출 권장")
    print(f"  이유: CV 안전 구간, Gap 관리 가능")
elif final_cv >= 16.0:
    print(f"  ⚠️  신중한 제출 권장")
    print(f"  이유: CV 경계 구간, Gap 증가 가능")
else:
    print(f"  ❌ 제출 보류 권장")
    print(f"  이유: 과적합 위험 높음 (CV < 16.0)")
