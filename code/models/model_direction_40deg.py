"""
K리그 패스 좌표 예측 - Direction 40도 간격

Week 2 첫 실험: Direction 각도 조정
- 6x6 Zone + 40도 간격 Direction (기존 45도)
- min_samples: 25
...
print("=" * 80)
print("K리그 패스 좌표 예측 - Direction 40도 간격")
print("=" * 80)
...
# =============================================================================
# 3. Zone 및 Direction 함수 (40도 간격)
# =============================================================================
print("\n[3] Zone 및 Direction 함수 정의 (40도 간격)...")
...
def get_direction_40deg(prev_dx, prev_dy):
    """40도 간격 8방향 분류 (기존 45도 → 40도)"""
    ...
    if -20 <= angle_deg < 20:
        ...
        return 'back_up'
    elif angle_deg >= 140 or angle_deg < -140:
        return 'backward'
    elif -140 <= angle_deg < -100:
        return 'back_down'
    elif -100 <= angle_deg < -60:
        return 'down'
    else:
        return 'forward_down'
...
def build_model_40deg(df, min_samples):
    """6x6 Zone + 40도 Direction 모델"""
    df = df.copy()
    df['zone'] = df.apply(lambda r: get_zone_6x6(r['start_x'], r['start_y']), axis=1)
    df['direction'] = df.apply(lambda r: get_direction_40deg(r['prev_dx'], r['prev_dy']), axis=1)
...
def predict_40deg(row, model):
    """예측"""
    zone = get_zone_6x6(row['start_x'], row['start_y'])
    direction = get_direction_40deg(row['prev_dx'], row['prev_dy'])
...
for fold, (train_idx, val_idx) in enumerate(gkf.split(train_last, groups=game_ids)):
    ...
    # 모델 구축
    model = build_model_40deg(train_fold, min_samples)

    # 예측
    predictions = val_fold.apply(lambda r: predict_40deg(r, model), axis=1)
...
final_model = build_model_40deg(train_last, min_samples)

predictions = test_last.apply(lambda r: predict_40deg(r, final_model), axis=1)
...
submission.to_csv('/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_direction_40deg.csv', index=False)

print("  submission_direction_40deg.csv 저장 완료")
...
print(f"  Direction: 40도 간격 (기존 45도)")
print(f"  min_samples: {min_samples}")
...
print(f"  40도 Direction:   {public_estimate:.4f} (예상)")
