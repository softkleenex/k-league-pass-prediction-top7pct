...
근거:
- 8direction_safe (단일 모델)가 CV 16.28 달성
- safe_fold13 (앙상블)은 CV 16.34
- 단일 모델이 CV 더 낮을 가능성
...
submission.to_csv('/mnt/c/LSJ/dacon/dacon/kleague-algorithm/submission_6x6_single.csv', index=False)

print("  submission_6x6_single.csv 저장 완료")
...
print(f"  6x6 단일:         {public_estimate:.4f} (예상)")
print(f"  개선:             {16.3639 - public_estimate:+.4f}")
print(f"")
print(f"  8direction_safe:  16.3574 (CV 16.28)")
print(f"  6x6 단일:         {public_estimate:.4f} (CV {fold13_cv:.4f})")
