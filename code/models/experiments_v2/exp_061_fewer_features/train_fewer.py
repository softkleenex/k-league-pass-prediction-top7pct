"""
exp_061: Fewer Features (Top 15개만)

핵심 가설: 피처가 많으면 과적합 → Gap 양수 → Public 악화
- exp_047: 37개 피처, Gap -0.18 (좋음)
- 새 시도: 15개 피처, Gap -0.3+ 목표
"""
...
def main():
    print("=" * 70)
    print("exp_061: Fewer Features (Top 15)")
    print("=" * 70)
    ...
    output_path = SUBMISSION_DIR / f"submission_fewer15_cv{cv:.2f}.csv"
    submission.to_csv(output_path, index=False)
    print(f"  저장: {output_path}")

    print("\n" + "=" * 70)
    print(f"CV: {cv:.4f} (15개 피처)")
    print("목표: Gap -0.3+ → Public 14.0 이하")
    print("=" * 70)
