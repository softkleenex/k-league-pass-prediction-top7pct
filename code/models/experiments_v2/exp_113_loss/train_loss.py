"""
exp_113: Alternative Loss Functions
- MAE (baseline)
- RMSE
- Huber (delta=1.0, 1.5, 2.0)
- Quantile (0.5)
"""
import pandas as pd
import numpy as np
...
def main():
    print("="*60)
    print("exp_113: Alternative Loss Functions")
    print("="*60)
    ...
    loss_configs = [
        {'name': 'MAE', 'loss': 'MAE'},
        {'name': 'RMSE', 'loss': 'RMSE'},
        {'name': 'Huber_1.0', 'loss': 'Huber:delta=1.0'},
        {'name': 'Huber_1.5', 'loss': 'Huber:delta=1.5'},
        {'name': 'Huber_2.0', 'loss': 'Huber:delta=2.0'},
        {'name': 'Quantile_0.5', 'loss': 'Quantile:alpha=0.5'},
    ]
    ...
    for config in loss_configs:
        ...
        for seed in [42, 123, 456]:
            params = {'iterations': 1000, 'depth': 8, 'learning_rate': 0.05,
                      ...
                      'early_stopping_rounds': 50, 'loss_function': loss}
            ...
            for train_idx, val_idx in folds:
                ...
                # Determine eval metric based on loss
                eval_metric = 'MAE' if loss != 'RMSE' else 'RMSE'

                m_dx.fit(X[train_idx], y_dx[train_idx],
                        eval_set=(X[val_idx], y_dx[val_idx]),
                        use_best_model=True)
                m_dy.fit(X[train_idx], y_dy[train_idx],
                        eval_set=(X[val_idx], y_dy[val_idx]),
                        use_best_model=True)

                pred_dx = m_dx.predict(X[val_idx])
                pred_dy = m_dy.predict(X[val_idx])

                # Always evaluate with Euclidean distance
