"""
exp_054: Model Comparison
CatBoost vs LightGBM vs XGBoost 비교
"""
...
def run_cv_catboost(X, y, groups):
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups), 1):
        ...
        model_x.fit(X[train_idx], y[train_idx, 0], eval_set=(X[val_idx], y[val_idx, 0]), use_best_model=True)
        model_y.fit(X[train_idx], y[train_idx, 1], eval_set=(X[val_idx], y[val_idx, 1]), use_best_model=True)
...
def run_cv_lightgbm(X, y, groups):
    """LightGBM CV"""
    gkf = GroupKFold(n_splits=3)
    params = {
        'n_estimators': 1000, 'max_depth': 8, 'learning_rate': 0.05,
        'reg_lambda': 3.0, 'random_state': 42, 'verbose': -1, 'n_jobs': -1
    }
...
def run_cv_xgboost(X, y, groups):
    """XGBoost CV"""
    gkf = GroupKFold(n_splits=3)
    params = {
        'n_estimators': 1000, 'max_depth': 8, 'learning_rate': 0.05,
        'reg_lambda': 3.0, 'random_state': 42, 'verbosity': 0, 'n_jobs': -1
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups), 1):
        model_x = xgb.XGBRegressor(**params, early_stopping_rounds=50)
        model_y = xgb.XGBRegressor(**params, early_stopping_rounds=50)

        model_x.fit(X[train_idx], y[train_idx, 0],
                   eval_set=[(X[val_idx], y[val_idx, 0])], verbose=False)
        model_y.fit(X[train_idx], y[train_idx, 1],
                   eval_set=[(X[val_idx], y[val_idx, 1])], verbose=False)
...
def main():
    print("=" * 70)
    print("exp_054: Model Comparison (CatBoost vs LightGBM vs XGBoost)")
    print("=" * 70)
    ...
    print("\n--- CatBoost ---")
    cv_cat, models_cat = run_cv_catboost(X, y, groups)
    print(f"  CV: {cv_cat:.4f}")

    print("\n--- LightGBM ---")
    cv_lgb, models_lgb = run_cv_lightgbm(X, y, groups)
    print(f"  CV: {cv_lgb:.4f}")

    print("\n--- XGBoost ---")
    cv_xgb, models_xgb = run_cv_xgboost(X, y, groups)
    ...
    for mx, my in models:
        pred_x_all += mx.predict(X_test) / len(models)
        pred_y_all += my.predict(X_test) / len(models)
    ...
    output_path = SUBMISSION_DIR / f"submission_{best_model_name.lower()}_cv{best_cv:.2f}.csv"
