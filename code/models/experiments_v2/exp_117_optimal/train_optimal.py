"""
exp_117: Optimal Settings Combination
- LR=0.01, iterations=3000 (from exp_115)
- +ema_momentum_y feature (from exp_114)
- MAE loss (from exp_113)
- L2=150 (from exp_109)
"""
import pandas as pd
import numpy as np
...
def create_features(df, add_momentum_y=False):
...
def main():
    print("="*60)
    print("exp_117: Optimal Settings Combination")
    print("="*60)
    print("Settings:")
    print("  - LR=0.01, iterations=3000 (from exp_115)")
    print("  - +ema_momentum_y feature (from exp_114)")
    print("  - MAE loss (from exp_113)")
    print("  - L2=150 (from exp_109)")
    print("="*60)

    train_df = pd.read_csv(DATA_DIR / 'train.csv')
    train_df = create_features(train_df, add_momentum_y=True)
    train_last = train_df.groupby('game_episode').last().reset_index()
    print(f"Episodes: {len(train_last)}")

    FEATURES_16 = TOP_15 + ['ema_momentum_y']
    ...
    # Test configurations
    configs = [
        {'name': 'Baseline (LR=0.05, 15 feats)', 'lr': 0.05, 'iters': 1000, 'feats': TOP_15},
        {'name': 'Optimal LR only (LR=0.01)', 'lr': 0.01, 'iters': 3000, 'feats': TOP_15},
        {'name': 'Optimal + momentum_y (16 feats)', 'lr': 0.01, 'iters': 3000, 'feats': FEATURES_16},
    ]
    ...
    for config in configs:
        ...
        X_cur = train_last[feats].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32)

        all_scores = []
        for seed in [42, 123, 456, 789, 1000, 2024, 9999]:  # 7 seeds
            ...
            for train_idx, val_idx in folds:
                ...
                m_dx.fit(X_cur[train_idx], y_dx[train_idx], eval_set=(X_cur[val_idx], y_dx[val_idx]), use_best_model=True)
                m_dy.fit(X_cur[train_idx], y_dy[train_idx], eval_set=(X_cur[val_idx], y_dy[val_idx]), use_best_model=True)

                pred_dx = m_dx.predict(X_cur[val_idx])
                pred_dy = m_dy.predict(X_cur[val_idx])
