"""
exp_082: Combination Optimization
- 10-fold + lr=0.01
- 10-fold + multi-seed
- 10-fold + lr=0.01 + multi-seed
"""
import pandas as pd
import numpy as np
...
def run_experiment(X, y_delta, y_abs, start_xy, groups, n_splits, lr, seeds, name):
    """Run experiment with given settings"""
    all_oof = []

    for seed in seeds:
        params = {
            'iterations': int(1000 * 0.05 / lr),  # Adjust iterations for lr
            'depth': 8,
            'learning_rate': lr,
            'l2_leaf_reg': 3.0,
            'random_state': seed,
...
def main():
    print("=" * 70)
    print("exp_082: Combination Optimization")
    print("=" * 70)
    ...
    # Baseline: 10-fold, lr=0.05, single seed
    print("\n[1] Baseline: 10-fold, lr=0.05, seed=42")
    cv = run_experiment(X, y_delta, y_abs, start_xy, groups,
                       n_splits=10, lr=0.05, seeds=[42], name="baseline")
    results['10fold_lr0.05_seed42'] = cv
    print(f"  CV: {cv:.4f}")

    # Combo 1: 10-fold + lr=0.01
    print("\n[2] Combo 1: 10-fold + lr=0.01")
    cv = run_experiment(X, y_delta, y_abs, start_xy, groups,
                       n_splits=10, lr=0.01, seeds=[42], name="combo1")
    results['10fold_lr0.01_seed42'] = cv
    print(f"  CV: {cv:.4f}")

    # Combo 2: 10-fold + multi-seed (3 seeds for speed)
    print("\n[3] Combo 2: 10-fold + multi-seed (3 seeds)")
    cv = run_experiment(X, y_delta, y_abs, start_xy, groups,
                       n_splits=10, lr=0.05, seeds=[42, 123, 456], name="combo2")
    results['10fold_lr0.05_3seeds'] = cv
    print(f"  CV: {cv:.4f}")

    # Combo 3: 10-fold + lr=0.01 + multi-seed
    print("\n[4] Combo 3: 10-fold + lr=0.01 + multi-seed (3 seeds)")
    cv = run_experiment(X, y_delta, y_abs, start_xy, groups,
                       n_splits=10, lr=0.01, seeds=[42, 123, 456], name="combo3")
    results['10fold_lr0.01_3seeds'] = cv
    print(f"  CV: {cv:.4f}")
    ...
    print("=" * 70)
    baseline_cv = results['10fold_lr0.05_seed42']
    ...
    print(f"  vs 10-fold baseline: {best_cv - baseline_cv:+.4f}")
    print("=" * 70)
...
def create_best_submission(best_name, cv, feature_cols, last_passes):
    """Create submission with best settings"""
    # Parse settings from name
    if 'lr0.01' in best_name:
        lr = 0.01
    else:
        lr = 0.05

    if '3seeds' in best_name:
        seeds = [42, 123, 456]
    else:
        seeds = [42]
    ...
    output_path = SUBMISSION_DIR / f"submission_combo_cv{cv:.2f}.csv"
