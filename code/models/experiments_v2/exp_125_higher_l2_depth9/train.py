"""exp_125: Higher L2 with Depth=9 (700, 800, 1000, 1500)"""
import pandas as pd
import numpy as np
...
def main():
    print("="*60)
    print("exp_125: Higher L2 with Depth=9")
    print("="*60)
    ...
    l2_values = [600, 700, 800, 1000, 1500, 2000]
    results = {}
    for l2 in l2_values:
        print(f"\nL2={l2}...")
        scores = []
        for seed in [42, 123, 456]:
            params = {'iterations': 3000, 'depth': 9, 'learning_rate': 0.01, 'l2_leaf_reg': float(l2), 'random_state': seed, 'verbose': 0, 'early_stopping_rounds': 100, 'loss_function': 'MAE'}
            fold_scores = []
    ...
    for k in sorted(results.keys(), key=lambda k: results[k][0]):
        ...
        print(f"  L2={k}: CV {cv:.4f} (+/- {std:.4f}){m}")
