...
def run_config(X, y_dx, y_dy, y_abs, start_xy, groups, config_name, params, n_seeds=7):
...
def main():
    print("=" * 70)
    print("exp_099: Heavy Regularization Test")
    print("=" * 70)
    ...
    configs = {
        ...
        'high_l2': {**baseline_params, 'l2_leaf_reg': 10.0},
        'very_high_l2': {**baseline_params, 'l2_leaf_reg': 30.0},
        'shallow': {**baseline_params, 'depth': 5},
        'shallow_high_l2': {**baseline_params, 'depth': 5, 'l2_leaf_reg': 10.0},
        'slow_lr': {**baseline_params, 'learning_rate': 0.02, 'iterations': 2000},
        'conservative': {**baseline_params, 'depth': 6, 'l2_leaf_reg': 10.0, 'learning_rate': 0.03, 'iterations': 1500},
    }

    results = {}
    for name, params in configs.items():
        print(f"\nTesting {name}...")
        cv = run_config(X, y_dx, y_dy, y_abs, start_xy, groups, name, params, n_seeds=3)
        results[name] = cv
    ...
    for name, cv in sorted(results.items(), key=lambda x: x[1]):
        diff = cv - results['baseline']
        print(f"  {name:20s}: CV {cv:.4f} ({diff:+.4f})")
    print("=" * 70)
