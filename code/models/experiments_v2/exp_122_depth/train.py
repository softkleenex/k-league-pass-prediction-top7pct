"""exp_122: Depth tuning with L2=300"""
import pandas as pd
import numpy as np
...
def main():
    print("="*60)
    print("exp_122: Depth tuning with L2=300")
    print("="*60)
    ...
    for depth in depths:
        ...
        for seed in [42, 123, 456]:
            params = {'iterations': 3000, 'depth': depth, 'learning_rate': 0.01, 'l2_leaf_reg': 300.0, 'random_state': seed, 'verbose': 0, 'early_stopping_rounds': 100, 'loss_function': 'MAE'}
            fold_scores = []
        ...
        cv = np.mean(scores)
        std = np.std(scores)
        results[depth] = (cv, std)
        print(f"  Depth={depth}: CV {cv:.4f} (+/- {std:.4f})")
    ...
    for k in sorted(results.keys(), key=lambda k: results[k][0]):
        ...
        print(f"  Depth={k}: CV {cv:.4f} (+/- {std:.4f}){m}")
