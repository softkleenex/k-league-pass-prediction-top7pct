...
def create_features(df, add_interactions=False):
    ...
    if add_interactions:
        # Angle * Distance interactions
        df['angle_dist'] = df['goal_angle'] * df['goal_distance']
        df['angle_to_goal'] = df['goal_angle'] / (df['dist_to_goal_line'] + 1)

        # Zone interaction (combined zone ID)
        df['zone_combined'] = df['zone_x'] * 6 + df['zone_y']

        # Direction of movement
        df['prev_direction'] = np.arctan2(df['prev_dy'], df['prev_dx'] + 1e-6)
        df['velocity_x'] = df['diff_x']  # x-component of velocity

        # Position relative to goal
        df['goal_pos_ratio'] = df['start_x'] / (105 - df['start_y'] + 1)

        # EMA momentum
        df['ema_momentum_x'] = df['ema_start_x'] - df['start_x']
...
INTERACTION_FEATURES = ['angle_dist', 'angle_to_goal', 'zone_combined',
                        'prev_direction', 'velocity_x', 'goal_pos_ratio',
                        'ema_momentum_x', 'ema_momentum_y']

def main():
    print("="*60)
    print("exp_114: Feature Interactions")
    print("="*60)

    train_df = pd.read_csv(DATA_DIR / 'train.csv')
    train_df = create_features(train_df, add_interactions=True)
    train_last = train_df.groupby('game_episode').last().reset_index()
    print(f"Episodes: {len(train_last)}")

    # Check interaction features
    print("\nInteraction features stats:")
    ...
    X_int = train_last[INTERACTION_FEATURES].fillna(0).replace([np.inf, -np.inf], 0).values.astype(np.float32)
    X_combined = np.hstack([X_base, X_int])
    ...
    cv_combined = evaluate(X_combined, "With Interactions (23 features)")

    # Test individual interactions
    print("\nTesting individual interactions...")
    for i, feat in enumerate(INTERACTION_FEATURES):
        X_single = np.hstack([X_base, X_int[:, i:i+1]])
        evaluate(X_single, f"+{feat} (16 features)")

    print("\n" + "="*60)
    print("RESULTS")
    print("="*60)
    print(f"  Baseline:          CV {cv_base:.4f}")
    print(f"  All Interactions:  CV {cv_combined:.4f}")
