"""
exp_104 Fast: Create submission with optimal weights
Simplified version: Cat(3 seeds) + XGB(1 seed) + LGB(1 seed)
Weights: Cat=0.60, XGB=0.15, LGB=0.25
"""
import pandas as pd
import numpy as np
...
# Best weights from exp_104
W_CAT, W_XGB, W_LGB = 0.60, 0.15, 0.25

def main():
    print("Creating submission with optimal weights (FAST version)...")
    print(f"Weights: Cat={W_CAT}, XGB={W_XGB}, LGB={W_LGB}")
    ...
    # CatBoost (3 seeds only)
    print("Training CatBoost (3 seeds)...")
    cat_pred_dx, cat_pred_dy = [], []
    ...
    # XGBoost (1 seed)
    print("Training XGBoost (1 seed)...")
    params = {'n_estimators': 1000, 'max_depth': 8, 'learning_rate': 0.05,
              'reg_lambda': 30.0, 'random_state': 42, 'verbosity': 0,
    ...
    for train_idx, val_idx in folds:
        ...
        fold_dy.append(m_dy.predict(X_test))
        del m_dx, m_dy
    xgb_dx = np.mean(fold_dx, axis=0)
    xgb_dy = np.mean(fold_dy, axis=0)
    gc.collect()
    print("XGBoost done!")

    # LightGBM (1 seed)
    print("Training LightGBM (1 seed)...")
    params = {'n_estimators': 1000, 'max_depth': 8, 'learning_rate': 0.05,
              'reg_lambda': 30.0, 'random_state': 42, 'verbosity': -1, 'objective': 'mae'}
    fold_dx, fold_dy = [], []
    for train_idx, val_idx in folds:
        m_dx = lgb.LGBMRegressor(**params)
        m_dy = lgb.LGBMRegressor(**params)
        m_dx.fit(X_train[train_idx], y_dx[train_idx], eval_set=[(X_train[val_idx], y_dx[val_idx])])
        m_dy.fit(X_train[train_idx], y_dy[train_idx], eval_set=[(X_train[val_idx], y_dy[val_idx])])
        fold_dx.append(m_dx.predict(X_test))
        fold_dy.append(m_dy.predict(X_test))
        del m_dx, m_dy
    lgb_dx = np.mean(fold_dx, axis=0)
    lgb_dy = np.mean(fold_dy, axis=0)
    gc.collect()
    print("LightGBM done!")

    # Ensemble
    print("Creating ensemble...")
    final_dx = W_CAT * cat_dx + W_XGB * xgb_dx + W_LGB * lgb_dx
    final_dy = W_CAT * cat_dy + W_XGB * xgb_dy + W_LGB * lgb_dy
    ...
    output_path = SUBMISSION_DIR / "submission_ensemble_opt_cv13.50.csv"
