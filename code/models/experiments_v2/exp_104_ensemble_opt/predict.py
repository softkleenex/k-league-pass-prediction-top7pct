"""
exp_104: Create submission with optimal weights
Best weights: Cat=0.60, XGB=0.15, LGB=0.25 (L2=30)
CV: 13.5024
"""
import pandas as pd
import numpy as np
...
def main():
    print("Creating submission with optimal weights...")
    ...
    for seed in SEED_POOL[:5]:
        ...
        for train_idx, val_idx in folds:
            ...
            fold_dy.append(m_dy.predict(X_test))
            del m_dx, m_dy
        lgb_pred_dx.append(np.mean(fold_dx, axis=0))
        lgb_pred_dy.append(np.mean(fold_dy, axis=0))
        gc.collect()
    lgb_dx = np.mean(lgb_pred_dx, axis=0)
    lgb_dy = np.mean(lgb_pred_dy, axis=0)
