"""
exp_007: 모델 앙상블 + 추가 Lag Features

테스트:
1. 추가 Lag Features (lag 2, 3)
2. Model Ensembling (CatBoost + LightGBM + XGBoost)
3. Target Encoding (zone별 평균 end_x, end_y)
...
def run_cv_ensemble(X, y, groups):
    """Ensemble (CatBoost + LightGBM + XGBoost) CV"""
    gkf = GroupKFold(n_splits=3)
    ...
    for fold, (train_idx, val_idx) in enumerate(gkf.split(X, y, groups), 1):
        ...
        # Simple Average
        pred_x = (pred_cat_x + pred_lgb_x + pred_xgb_x) / 3
        pred_y = (pred_cat_y + pred_lgb_y + pred_xgb_y) / 3
...
def main():
    print("=" * 70)
    print("exp_007: 모델 앙상블 + 추가 Lag Features")
    print("=" * 70)
    ...
    # exp_006 ALL Advanced features
    ALL_ADVANCED = BASELINE + EMA + POSITION + DIFF + ROLLING + VELOCITY + RELATIVE + INTERACTIONS + MOMENTUM
    ...
    # 실험
    print("\n" + "=" * 70)
    print("[실험 1] exp_006 ALL Advanced (CatBoost) - 기준")
    print("=" * 70)
    X = last_passes[ALL_ADVANCED].fillna(0).replace([np.inf, -np.inf], 0).values
    cv_baseline = run_cv_catboost(X, y, groups)
    print(f"  CV: {cv_baseline:.4f}")

    print("\n" + "=" * 70)
    print("[실험 2] + 추가 Lag Features (CatBoost)")
    print("=" * 70)
    FEATURES_WITH_LAG = ALL_ADVANCED + LAG_FEATURES
    X = last_passes[FEATURES_WITH_LAG].fillna(0).replace([np.inf, -np.inf], 0).values
    cv_lag = run_cv_catboost(X, y, groups)
    print(f"  CV: {cv_lag:.4f}  ({cv_baseline - cv_lag:+.4f})")

    print("\n" + "=" * 70)
    print("[실험 3] ALL Advanced (Ensemble: Cat+LGB+XGB)")
    print("=" * 70)
    X = last_passes[ALL_ADVANCED].fillna(0).replace([np.inf, -np.inf], 0).values
    cv_ensemble = run_cv_ensemble(X, y, groups)
    print(f"  CV: {cv_ensemble:.4f}  ({cv_baseline - cv_ensemble:+.4f})")

    print("\n" + "=" * 70)
    print("[실험 4] ALL + Lag Features (Ensemble)")
    print("=" * 70)
    X = last_passes[FEATURES_WITH_LAG].fillna(0).replace([np.inf, -np.inf], 0).values
    cv_ensemble_lag = run_cv_ensemble(X, y, groups)
    print(f"  CV: {cv_ensemble_lag:.4f}  ({cv_baseline - cv_ensemble_lag:+.4f})")
    ...
    print("=" * 70)
    results = {
        "exp_006 ALL (CatBoost)": cv_baseline,
        "+ Lag Features (CatBoost)": cv_lag,
        "ALL (Ensemble)": cv_ensemble,
        "+ Lag (Ensemble)": cv_ensemble_lag,
    }
